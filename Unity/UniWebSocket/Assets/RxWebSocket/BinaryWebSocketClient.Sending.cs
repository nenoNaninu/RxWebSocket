...
namespace RxWebSocket
{
    public partial class BinaryWebSocketClient
    {
        ...
        public void Send(byte[] message)
        {
            ...
            {
                _binaryMessageQueue.Add(new ArraySegment<byte>(message));
            }
            else
            {
        ...
        public void Send(ArraySegment<byte> message)
        {
            if (ValidationUtils.ValidateInput(message))
            {
                _binaryMessageQueue.Add(message);
            }
            else
            {
        ...
        private async Task SendBinaryFromQueue()
        {
            try
            {
                foreach (var message in _binaryMessageQueue.GetConsumingEnumerable(_cancellationAllJobs.Token))
                {
                    try
                    {
