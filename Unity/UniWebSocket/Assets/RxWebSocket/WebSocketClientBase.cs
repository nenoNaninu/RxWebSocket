...
namespace RxWebSocket
{
    ...
    /// 
    /// </summary>
    /// <typeparam name="T">Type of MessageQueue for sending</typeparam>
    public abstract class WebSocketClientBase<T>: IWebSocketClient
    {
        #region Member variable with state.
        protected readonly ILogger _logger;

        protected readonly MemoryPool _memoryPool;

        protected readonly Func<Uri, CancellationToken, Task<WebSocket>> _connectionFactory;

        protected readonly AsyncLock _sendLocker = new AsyncLock();
        protected readonly AsyncLock _closeLocker = new AsyncLock();

        protected readonly Subject<byte[]> _binaryMessageReceivedSubject = new Subject<byte[]>();
        protected readonly Subject<string> _textMessageReceivedSubject = new Subject<string>();

        protected readonly Subject<CloseMessage> _closeMessageReceivedSubject = new Subject<CloseMessage>();
        protected readonly Subject<WebSocketExceptionDetail> _exceptionSubject = new Subject<WebSocketExceptionDetail>();
        
        protected readonly BlockingCollection<T> _sendMessageQueue = new BlockingCollection<T>();

        protected readonly CancellationTokenSource _cancellationCurrentJobs = new CancellationTokenSource();
        protected readonly CancellationTokenSource _cancellationAllJobs = new CancellationTokenSource();
        ...
        public WebSocketClientBase(Uri url, Func<ClientWebSocket> clientFactory = null)
        ...
        /// <param name="clientFactory">Optional factory for native ClientWebSocket, use it whenever you need some custom features (proxy, settings, etc)</param>
        public WebSocketClientBase(Uri url, ILogger logger, Func<ClientWebSocket> clientFactory = null)
            : this(url, MakeConnectionFactory(clientFactory))
        {
            _logger = logger;
        ...
        /// <param name="logger"></param>
        /// <param name="clientFactory">Optional factory for native ClientWebSocket, use it whenever you need some custom features (proxy, settings, etc)</param>
        public WebSocketClientBase(Uri url, int initialMemorySize, ILogger logger = null, Func<ClientWebSocket> clientFactory = null)
            : this(url, MakeConnectionFactory(clientFactory))
        {
            _logger = logger;
        ...
        /// <param name="logger"></param>
        /// <param name="clientFactory">Optional factory for native ClientWebSocket, use it whenever you need some custom features (proxy, settings, etc)</param>
        public WebSocketClientBase(Uri url, int initialMemorySize, int receiveBufferSize, ILogger logger = null, Func<ClientWebSocket> clientFactory = null)
            : this(url, MakeConnectionFactory(clientFactory))
        {
            _logger = logger;
        ...
        private WebSocketClientBase(Uri url, Func<Uri, CancellationToken, Task<WebSocket>> connectionFactory)
        {
        ...
        public WebSocketClientBase(Uri url, int initialMemorySize, ILogger logger, Func<Uri, CancellationToken, Task<WebSocket>> connectionFactory)
        {
            if (!ValidationUtils.ValidateInput(url))
            {
                throw new WebSocketBadInputException($"url is null. Please correct it.");
        ...
        /// <param name="logger"></param>
        /// <param name="connectionFactory">An optional factory for creating and connecting native Websockets. The method should return connected websocket.</param>
        public WebSocketClientBase(Uri url, int initialMemorySize, int receiveBufferSize, ILogger logger, Func<Uri, CancellationToken, Task<WebSocket>> connectionFactory)
        {
            if (!ValidationUtils.ValidateInput(url))
            {
                throw new WebSocketBadInputException($"url is null. Please correct it.");
        ...
        public WebSocketClientBase(WebSocket connectedSocket, ILogger logger = null)
        {
            Url = null;
        ...
        /// <param name="logger"></param>
        /// <param name="connectedSocket">Already connected socket.</param>
        public WebSocketClientBase(WebSocket connectedSocket, int initialMemorySize, ILogger logger = null)
        {
            Url = null;
        ...
        /// <param name="logger"></param>
        /// <param name="connectedSocket">Already connected socket.</param>
        public WebSocketClientBase(WebSocket connectedSocket, int initialMemorySize, int receiveBufferSize, ILogger logger = null)
        {
            Url = null;
        ...
        // protected abstract void StartBackgroundThreadForSendingMessage();

        public abstract void Send(string message);

        public abstract void Send(byte[] message);

        public abstract void Send(byte[] message, WebSocketMessageType messageType);

        public abstract void Send(ArraySegment<byte> message);

        public abstract void Send(ArraySegment<byte> message, WebSocketMessageType messageType);

        public abstract Task SendInstant(string message);
        
        public abstract Task SendInstant(byte[] message);
        public abstract Task SendInstant(byte[] message,  WebSocketMessageType messageType);
        
        public abstract Task SendInstant(ArraySegment<byte> message);
        public abstract Task SendInstant(ArraySegment<byte> message,  WebSocketMessageType messageType);
        
        protected abstract Task SendInternal(T message);
        ...
        protected async Task<bool> ConnectAndStartListeningInternal(Uri uri, CancellationToken token)
        {
            try
            {
        ...
        protected static Func<Uri, CancellationToken, Task<WebSocket>> MakeConnectionFactory(Func<ClientWebSocket> clientFactory)
        {
        ...
        protected async Task Listen(WebSocket client, CancellationToken token)
        {
            try
            {
                do
        ...
        protected async Task SendInternalSynchronized(T message)
        {
        ...
        protected void StartBackgroundThreadForSendingMessage()
        {
